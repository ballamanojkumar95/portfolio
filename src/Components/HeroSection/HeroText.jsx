...
const HeroText = () => {
 ...
 // const [typeds,setTyped] = useState(null);
  ...
  useEffect(() => {
    const options = {
      strings: [
        "<span class='text-[15px]'>am Frontend <span class='text-orange '>Web developer.</span></span>"
      ],
      typeSpeed: 30,
      backSpeed: 40,
      loop: true,
  ...
  return (
    <div className='flex flex-col gap-4 h-full justify-center md:text-left sm:text-center'>
      <motion.h2  variants={fadeIn('down',0.2)} initial="hidden" viewport={{once:false,amount:0}} whileInView="show" className='lg:text-2xl sm:text-xl uppercase text-cyan element'><span className='text-[15px]'>I </span><span ref={el}></span></motion.h2>
      ...
      <div className='flex mt-2 gap-2 items-center'>
       ...
       <a href="https://github.com/ballamanojkumar95" target='_blank'> <FiGithub className='text-3xl text-cyan hover:text-orange transition-all duration-500'/></a>
       <button onClick={downloadResume}> <FaRegFilePdf className='text-3xl text-cyan hover:text-orange transition-all duration-500'/></button>
      </div>
    </div>
  )
}
